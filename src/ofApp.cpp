...
void ofApp::setupUI(ofJson madmapperJson) {
    ...
    if (madmapperJson["CONTENTS"]["surfaces"]["CONTENTS"]["Mapping"].is_object())
        fadeMasterVideo = madOscQuery.createParameter(
                                                      madmapperJson["CONTENTS"]["surfaces"]["CONTENTS"]["Mapping"]["CONTENTS"]
                                                      ["opacity"]);
    else
        fadeMasterVideo = madOscQuery.createParameter(
                                                      madmapperJson["CONTENTS"]["master"]["CONTENTS"]["master_video_level"]);
    
    fadeMasterVideo->linkMidiComponent(faderport16.midiComponents["fader_M_video"]);
    
    
    // DMX/Lighting Master
    if (madmapperJson["CONTENTS"]["surfaces"]["CONTENTS"]["Lighting"].is_object())
        fadeMasterDMX = madOscQuery.createParameter(
                                                      madmapperJson["CONTENTS"]["surfaces"]["CONTENTS"]["Lighting"]["CONTENTS"]
                                                      ["opacity"]);
    else
        fadeMasterDMX = madOscQuery.createParameter(
                                                madmapperJson["CONTENTS"]["master"]["CONTENTS"]["master_dmx_level"]);
    fadeMasterDMX->linkMidiComponent(faderport16.midiComponents["fader_M_dmx"]);
    
    
    // Engine Speed Master
    fadeEngingeSpeed = madOscQuery.createParameter(
                                                madmapperJson["CONTENTS"]["master"]["CONTENTS"]["engine_speed"]);
